                            ...
                            PrimaryButton(
                                onClick = {
                                    viewModel.reorder(order) {
                                        android.widget.Toast.makeText(context, "Order items added to cart", android.widget.Toast.LENGTH_SHORT).show()
                                        onNavigateToCart()
                                    }
                                },
...
fun OrderReceiptTotals(order: Order) {
    Card(
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            Spacer(modifier = Modifier.height(12.dp))
            androidx.compose.material3.HorizontalDivider(color = VAL_BACKGROUND)
            Spacer(modifier = Modifier.height(12.dp))
            Row(
                modifier = Modifier.fillMaxWidth(),
                horizontalArrangement = Arrangement.SpaceBetween
